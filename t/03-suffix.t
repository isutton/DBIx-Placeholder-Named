...
{
    my $query = q{INSERT INTO test VALUES (id__, name__)};
    my $sth   = $dbh->prepare($query);
    ok( $sth, $query );
...
{
    my $query =
      q{UPDATE test SET id = new_id__, name = new_name__ WHERE id = id__};
    my $sth = $dbh->prepare($query);
    ok( $sth, $query );
...
{
    my $query = q{SELECT id, name FROM test WHERE id = id__};
    my $sth   = $dbh->prepare($query);
    ok( $sth, $query );
...
{
    my $query =
      q{INSERT INTO test (id, name) VALUES (id__, '0000-00-00 11:11:11')};
    my $sth = $dbh->prepare($query);
    ok( $sth, $query );
    my $rv = $sth->execute( { id => 2, } );
    ok( $rv, $query . ': execute()' );
